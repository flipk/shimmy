...
main()
{
    ...
    s2c.set_type(shimmyTest::S2C_ICD_VERSION);
    s2c.mutable_icd_version()->set_version(shimmyTest::ICD_VERSION);
    ...
    while (!done)
    {
        ...
        switch (c2s.type())
        {
        case shimmyTest::C2S_ICD_VERSION:
        ...
        case shimmyTest::C2S_CRASH_CMD:
            printf("%d: child: told to exit\n", Shimmy::get_tid());
            done = true;
            break;

        case shimmyTest::C2S_DATA:
            printf("%d: child: got DATA from parent\n", Shimmy::get_tid());
            s2c.set_type(shimmyTest::S2C_DATA);
